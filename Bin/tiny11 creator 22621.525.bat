...
if exist %DriveLetter%\sources\install.wim (
    set TINY11INSTALL_IMAGE=install.wim
) else (
    set TINY11INSTALL_IMAGE=install.esd
)

md c:\tiny11
echo Copying Windows image...
xcopy.exe /E /I /H /R /D /J %DriveLetter% c:\tiny11
echo Copy complete!
echo Getting image information from %TINY11INSTALL_IMAGE%
pause
dism /Get-WimInfo /wimfile:c:\tiny11\sources\%TINY11INSTALL_IMAGE%
set index=
...
echo.
md c:\scratchdir
dism /mount-image /imagefile:c:\tiny11\sources\%TINY11INSTALL_IMAGE% /index:%index% /mountdir:c:\scratchdir
...
Dism /Export-Image /SourceImageFile:c:\tiny11\sources\%TINY11INSTALL_IMAGE% /SourceIndex:%index% /DestinationImageFile:c:\tiny11\sources\install2.wim /compress:max
del c:\tiny11\sources\%TINY11INSTALL_IMAGE%
